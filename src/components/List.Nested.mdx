...
When using a listAtom within a listAtom, you can use the `List.Nested` component to render the nested list. The `atom` prop is required and should be set to the nested list atom.
...
function Example() {
  return (
    <List.Item>
      {({ fields }) => (
        <List.Nested atom={fields.yourNestedListAtom}>
